...
if (typeof window !== 'undefined') {
  onIdTokenChanged(auth, async (fbUser) => {
    ...
    if (fbUser) {
      ...
      await fetch('/projects/api/auth/session', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ idToken }),
      });

      // Sync Firebase user to PostgreSQL (upsert)
      await fetch('/projects/api/auth/sync-user', { method: 'POST' });
    }
  });
}
...
export async function logout(): Promise<void> {
  ...
  await fetch('/projects/api/auth/session', { method: 'DELETE' });
}
