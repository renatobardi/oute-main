/**
 * Client-side authentication utilities for 99_home
 */
 ...
 * Calls POST /api/auth?action=login on 01_auth-profile
 */
export async function login(email: string, password: string): Promise<void> {
  const authUrl = import.meta.env.VITE_AUTH_SERVICE_URL ?? 'http://localhost:3001';

  const response = await fetch(`${authUrl}/api/auth?action=login`, {
    method: 'POST',
